...
def key_gen():  # normal key generation
	q = getPrime(256)  # gen q
	while True:  # gen p
...
def sign(message, pubkey, privkey):
	p, q, g, y = pubkey
	x = privkey
	k = pow(y, x, g) * random.randrange(1, 512) % q # k generation schedule is not normal
	r = pow(g, k, p) % q
	s = inverse(k, q) * (hash(message) + x * r) % q
	return r, s

def verify(message, signature, pubkey):  # verify schedule is normal
	p, q, g, y = pubkey
	r, s = signature
