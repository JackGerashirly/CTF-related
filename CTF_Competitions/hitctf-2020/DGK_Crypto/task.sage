...
class DGK(object):
    ...
    def keygen(self):
        vpq_bits = 160 # ==t
        vp = random_prime(2**vpq_bits,False,2**(vpq_bits-1)) # 2^159 ~ 2^160
        vq = random_prime(2**vpq_bits,False,2**(vpq_bits-1)) # 2^159 ~ 2^160
        u = 1 << self.lbits # 2^64
        
        r1bits = self.nbits / 2 - vpq_bits - self.lbits # 288
        flag = 0
        while not flag: # generate p
            r1 = random_prime(2**r1bits,False,2**(r1bits-1))
            p = (u*vp*r1) + 1
            flag = is_prime(p)
            
        r2bits = self.nbits / 2 - vpq_bits - 1 - self.lbits # ??????-1
        flag = 0
        while not flag: # generate q
        ...
        g = (qinv * q * xp + pinv * p * vq) % n  # CRT ???
        g = pow(g, r1*r2, n)
        h = pow(g, r1*r2*u, n)

        return (n, u, g, h), (vp, vq, p, q)

    def encrypt(self, pt): # normal encryption
...
if __name__ == "__main__":
    ...
    assert len(FLAG)  == 72  # length of flag
    dgk = DGK(1024, 32)
    chunks, chunk_size = len(FLAG), 8
    flag_num = [bytes_to_long(FLAG[i:i+chunk_size]) for i in range(0, chunks, chunk_size)] # encrypt with chunks
