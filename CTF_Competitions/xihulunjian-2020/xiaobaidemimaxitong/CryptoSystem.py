...
class Task(BaseRequestHandler):
    ...
    def encryption(self,message):
        sm = sm4_encryption(self.key) # initial
        sm.__key__expand__()  # key generation
        en_message=sm.encrypt_cbc(message,self.key) # encrypt the message
        self.c=en_message
        return en_message
    ...
    def handle(self):
        self.key=os.urandom(16) # key is fixed
        self.c=""
        menu = '''
1. Dragon King
2. encryption
        ...
        try:
            while True:
                ...
                if choice=="1":
                    ...
                    continue
                if choice=="2":
                    self.request.send("What message do you want to encrypt:".encode()) # send message
                    message=self.recvline(BUFSIZE) # get message
                    en_message=self.encryption(message)  # encrypt message
                ...
                if choice=="3":
                    ...
                    iv=long_to_bytes(eval('0x'+iv)) # iv is accepted in hex format
