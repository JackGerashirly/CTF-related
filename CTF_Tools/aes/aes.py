...
class Aes:
    def __init__(self):
        ...
        self.rcon_box = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36]
    ...
    def g_function(self, g_f_key, r):
        ...
        temp_box = []
        for i in range(1, 4):
            temp = g_f_key[8 * i:8 * i + 8]
            temp_box.append(temp)
        temp_box.append(g_f_key[0:8])  # Compensate the First Block
        ...
        if r >= 10:
            ...
            for i in range(0, (8 - len(temp_one))):
                temp_one = "0" + temp_one
    ...
    def generate_key(self, g_key):
        ...
        for i in range(0, (128 - len(bin_key))):
            bin_key = "0" + bin_key
    ...
    def row_shift_permutation(self, r):

        # Shift
        for i in range(r):
            temp = self.plaintext_box[r * 4]
            self.plaintext_box[r * 4] = self.plaintext_box[r * 4 + 1]
            self.plaintext_box[r * 4 + 1] = self.plaintext_box[r * 4 + 2]
            self.plaintext_box[r * 4 + 2] = self.plaintext_box[r * 4 + 3]
            self.plaintext_box[r * 4 + 3] = temp

    def gmult(self, a, b):  # a is from the box, b is plaintext
        if a == 1:
            return int(b, 2)
        if a == 2:
            if b[0]:
        ...
        if a == 3:
            ...
            if b[0]:
                temp_two = (b + "0")[1:]
                temp_two = int(temp_two, 2) ^ 0x1b
                return temp_one ^ temp_two
            else:
                temp_two = (b + "0")[1:]
                temp_two = int(temp_two, 2)
                return temp_one ^ temp_two
    ...
    def column_confusion_permutation(self):
        ...
        for i in range(4):
            for j in range(4):
                temp_one = self.gmult(a[i][0], b[4 * 0 + j])
                temp_two = self.gmult(a[i][1], b[4 * 1 + j])
                temp_three = self.gmult(a[i][2], b[4 * 2 + j])
                temp_four = self.gmult(a[i][3], b[4 * 3 + j])
                temp = temp_one ^ temp_two ^ temp_three ^ temp_four
                temp = bin(temp)[2:]
                for k in range(0, (8 - len(temp))):
                    temp = "0" + temp
    ...
    def encrypt(self):
        ...
        plain_text = input("Please input your plaintext in Hex: ")

        # Change to 128-bit Binary
        bin_plaintext = bin(int(plain_text, 16))[2:]
        for i in range(0, (128 - len(bin_plaintext))):
        ...
        for i in range(10):
            ...
            for j in range((i + 1) * 4, (i + 2) * 4):
                round_key += self.round_key_box[i]
            for j in range(16):
                bin_plaintext += self.plaintext_box[j]
